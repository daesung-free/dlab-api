 ...
 * 이벤트별 발송 채널과 문구 템플릿.
 *
 * <p>문구 자체는 아직 운영팀과 확정되지 않은 블로커라(CLAUDE.md §4),
 * 지금은 채널 매핑과 변수 슬롯만 확정하고 문구는 비워둔 채 {@code contentConfirmed = false}로 둔다.
 * 문구가 미확정인 템플릿은 렌더링까지만 하고 실제 발송은 하지 않는다(SKIPPED).
 */
@Getter
@Entity
...
public class NotificationTemplate extends BaseTimeEntity {
    ...
    /** 모든 학생 관련 알림에 반드시 들어가야 하는 변수. 다자녀 학부모 구분용(CLAUDE.md §3). */
