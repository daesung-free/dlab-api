...
/**
 * 특강 유형 세분류 관리 (F-4.10-4 · 화면의 '유형' 컬럼).
 *
 * <p>단과·실전·해설이 여기 들어간다. 발주 회신이 <i>"추가할 수 있게"</i>였으므로
 * 코드에 박지 않고 관리자가 화면에서 넣는다.
 *
 * <p><b>특강/설명회({@code LectureType})와 다른 축이다</b> — 그쪽은 탭이고 이쪽은
 * 그 특강의 종류다. 설명회에는 붙지 않는다.
 */
@Tag(name = "관리자 · 특강 유형")
@RestController
...
public class AdminLectureCategoryController {
     ...
     *
     * @param activeOnly 기본은 전체다 — 관리 화면은 꺼둔 항목도 봐야 다시 켤 수 있다.
     *                   등록 폼 드롭다운은 {@code true}로 부른다
     */
    @GetMapping
