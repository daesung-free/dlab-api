...
public final class MealResponse {
     ...
     */
    public record MealItem(Long itemId, LocalDate date, String mealType, boolean cancelable) {

        public static MealItem from(MealOrderItem item) {
            return new MealItem(item.getId(), item.getMealDate(), item.getMealType().name(), true);
        }

        public static MealItem from(MealOrderItem item, boolean cancelable) {
            return new MealItem(item.getId(), item.getMealDate(), item.getMealType().name(), cancelable);
        }
    }
}
