...
--
-- ⚠️ 이 파일 이름을 바꾸지 말 것. 운영 DB 에 20260826.1000 으로 이미 적용돼 있다.
--    한 번 _1600 으로 바꿨다가 배포 리허설이 막혔다 —
--    "Detected applied migration not resolved locally: 20260826.1000".
--    Flyway 는 파일명에서 버전을 읽으므로, 적용된 뒤에 이름을 바꾸면
--    그 DB 는 이 마이그레이션이 사라진 것으로 본다.
--
