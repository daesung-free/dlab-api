...
| 승인 | **관리자 승인 필요.** 승인 전 앱 접근 완전 차단(PENDING→ACTIVE 이분법, 중간상태 없음) | **없음 — 즉시 ACTIVE** |

> **★ 학생 가입은 휴대폰번호로 중복을 차단한다.** 이미 승인요청을 넣은 학생이 다시 가입을 시도하면 휴대폰번호로 대조해 막는다 — `login_id` UNIQUE만으로는 부족하고, **PENDING 상태의 기존 요청까지 함께 검사**해야 한다(승인 전이라도 이미 신청한 사람이다).
>
> 이 규칙은 **신상기록부와 무관하다.** 회의록의 "회원가입 못하게…" 문장이 신상기록부 제출 조건으로 오해될 여지가 있었으나, 중복방지 로직을 설명한 것으로 확인됐다(S 참고).

> 학부모-자녀 관계 자체는 **검증하지 않는다** — 학생 고유ID를 아는 사람이면 누구나 연결 가능하다. 클라이언트가 인지·감수한 부분이므로 **추가 검증 로직을 임의로 만들지 말 것.**
...
## S. personal_record_card — 신상기록부 (F-4.11-9, I-17 — 양식 확정 대기)
...
> - ✅ **회원가입 연계 의혹 해소 — 신상기록부와 무관하다.** 회의록의 "회원가입 못하게…" 문장은 신상기록부 제출 조건이 아니라 **학생 회원가입 중복방지 로직**을 설명한 것이었다(A0 참고). 신상기록부 미작성은 `account.status`(가입승인)에 영향을 주지 않는다.
...
- **회원가입 연계 의혹** → 신상기록부와 무관, 학생 회원가입 중복방지 로직 설명이었던 것으로 확인